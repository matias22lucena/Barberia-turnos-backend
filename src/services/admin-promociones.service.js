...
const validarId = (valor) => {
  ...
  if (!Number.isInteger(id) || id <= 0) {
    const error = new Error("El id de la promoción no es válido");
...
export const listarPromocionesAdmin = async () => {
  return await obtenerPromocionesAdmin();
};

export const crearPromocionAdministrador = async ({
  servicioId,
  titulo,
  descripcion,
  precio,
  ...
  const datos = {
    servicioId: validarServicioId(servicioId),
  ...
  const promocionId = await crearPromocionAdmin(datos);

  return await obtenerPromocionPorId(promocionId);
};

export const editarPromocionAdmin = async ({
  promocionId,
  servicioId,
  titulo,
  descripcion,
  ...
  const idValidado = validarId(promocionId);

  const promocionExistente =
    await obtenerPromocionPorId(idValidado);
  ...
  await actualizarPromocionAdmin(datos);

  return await obtenerPromocionPorId(idValidado);
};

export const eliminarPromocionAdministrador = async (
  promocionId
) => {
  const idValidado = validarId(promocionId);
  ...
  await eliminarPromocionAdmin(idValidado);
};